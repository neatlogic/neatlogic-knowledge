...
//        }
    ...
    private List<String> handleText(JSONObject item, List<JSONObject> childItemList) {
    ...
    private List<KnowledgeDocumentLineVo> handleText(JSONObject item, List<JSONObject> childItemList, JSONArray unprocessedItems, List<String> allBlockIdList) {
        ...
        List<String> contentList = handleText(item, childItemList);
...
//                    content = content + "<ol>" + orderedListChildContent + "</ol>";
                    content = content + orderedListChildContent;
                }
    ...
    private String getOrderedListChildContent(JSONObject item, List<JSONObject> childItemList, JSONArray unprocessedItems, List<String> allBlockIdList) {
        ...
        for (int i = 0; i < directChildItemList.size(); i++) {
            ...
            FeiShuBlockType feiShuBlockType = FeiShuBlockType.getFeiShuBlockType(blockType);
            if (feiShuBlockType == FeiShuBlockType.ORDERED) {
                List<JSONObject> orderedList = new ArrayList<>();
                for (; i < directChildItemList.size(); i++) {
                    JSONObject orderedItem = directChildItemList.get(i);
                    if (!Objects.equals(orderedItem.getInteger("block_type"), FeiShuBlockType.ORDERED.getValue())) {
                        i--;
                        break;
                    }
                ...
                List<String> contentList = getOrderedListItemContent(orderedList, childItemList, unprocessedItems, allBlockIdList);
                builder.append("<ol>" + String.join("", contentList) + "</ol>");
                ...
                builder.append("<ul>" + String.join("", contentList) + "</ul>");
            } else if (feiShuBlockType == FeiShuBlockType.IMAGE) {
                builder.append("<ol><li>如图所示" + getImageHtml(childItem) + "</li></ol>");
                allBlockIdList.remove(childItem.getString("block_id"));
            } else if (feiShuBlockType == FeiShuBlockType.TEXT) {
                List<String> contentList = handleText(childItem, new ArrayList<>());
                builder.append("<ol><li><p>").append(String.join("", contentList)).append("</p></li></ol>");
                allBlockIdList.remove(childItem.getString("block_id"));
            } else {
    ...
    private String getImageHtml(JSONObject item) {
        JSONObject imageObj = handleImage(item);
        String url = imageObj.getString("url");
        if (StringUtils.isBlank(url)) {
            return "";
        }
        StringBuilder builder = new StringBuilder("<p><img src=\"");
        builder.append(url);
        builder.append("\"");
        Integer width = imageObj.getInteger("width");
        if (width != null) {
            builder.append(" width=\"").append(width).append("\"");
        }
        Integer height = imageObj.getInteger("height");
        if (height != null) {
        ...
        if (CollectionUtils.isNotEmpty(bulletList)) {
            ...
            for (JSONObject item : bulletList) {
                ...
                if (CollectionUtils.isNotEmpty(childItemList2)) {
                    ...
                    content = content + bulletListChildContent;
    ...
    private String getBulletListChildContent(JSONObject item, List<JSONObject> childItemList, JSONArray unprocessedItems, List<String> allBlockIdList) {
        ...
        for (int i = 0; i < directChildItemList.size(); i++) {
            ...
            if (feiShuBlockType == FeiShuBlockType.BULLET) {
                ...
                builder.append("<ul><li>如图所示" + getImageHtml(childItem) + "</li></ul>");
                allBlockIdList.remove(childItem.getString("block_id"));
            } else if (feiShuBlockType == FeiShuBlockType.TEXT) {
                List<String> contentList = handleText(childItem, new ArrayList<>());
                builder.append("<ul><li><p>").append(String.join("", contentList)).append("</p></li></ul>");
                allBlockIdList.remove(childItem.getString("block_id"));
            } else {
                unprocessedItems.add(childItem);
                allBlockIdList.remove(childItem.getString("block_id"));
...
//            logger.error("handleImage 方法中childItemList入参未处理,{}", JSONObject.toJSONString(childItemList));
    ...
    private String getFileName(JSONObject item) {
        JSONObject jsonObj = item.getJSONObject(FeiShuBlockType.FILE.getText());
        if (MapUtils.isNotEmpty(jsonObj)) {
    ...
    private List<KnowledgeDocumentLineVo> handleFile(JSONObject item, List<JSONObject> childItemList, JSONArray unprocessedItems, List<String> allBlockIdList) {
        List<KnowledgeDocumentLineVo> resultList = new ArrayList<>();
        String blockId = item.getString("block_id");
        Integer blockType = item.getInteger("block_type");
        KnowledgeDocumentLineVo knowledgeDocumentLineVo = new KnowledgeDocumentLineVo();
        knowledgeDocumentLineVo.setHandler("file");
        JSONObject configObj = new JSONObject();
        configObj.put("blockType", "file");
        configObj.put("blockUuid", blockId);
        configObj.put("feiShuBlockList", new JSONArray().fluentAdd(item));
        FeiShuBlockType feiShuBlockType = FeiShuBlockType.getFeiShuBlockType(blockType);
        JSONObject jsonObj = item.getJSONObject(feiShuBlockType.getText());
...
//                }
                ...
                lineList.addAll(lines);
            } else if (feiShuBlockType == FeiShuBlockType.FILE) {
                List<JSONObject> childItemList = collectChildItemList(item, itemMap);
                handledBlockIdList.addAll(collectChildItemBlockIdList(childItemList));
                List<KnowledgeDocumentLineVo> lines = handleFile(item, childItemList, unprocessedItems, allBlockIdList);
                lineList.addAll(lines);
